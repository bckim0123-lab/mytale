...
- Production negative-request smoke exposed unsupported upload formats being classified as retryable server failures. Follow-up validation now returns HTTP 415 for unsupported media and HTTP 400 for malformed multipart, with no provider call. Fourteen JSON/NDJSON regression cases, quality/generation tests, typecheck and targeted lint passed after this correction.
