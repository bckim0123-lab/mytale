...
import { ArrowLeft, BookOpen, Camera, Check, ChevronRight, ImagePlus, LockKeyhole, Mic, RotateCcw, Settings, ShieldCheck, Sparkles, Star, Trash2, Upload, Volume2, WandSparkles } from 'lucide-react';
...
export default function Home() {
  ...
  const load = (file?: File) => { if (!file) return; const r = new FileReader(); r.onload = () => setImage(String(r.result)); r.readAsDataURL(file); };
  const reset = () => { setImage(null); setPick(0); setScene(0); setPage(0); setStep('welcome'); };
  ...
  return <main className="app">
    ...
    {step === 'upload' && <section className="center upload"><span className="badge">첫 번째 마법</span><h2>그림을 보여 주세요!</h2><p>종이 전체가 잘 보이고, 밝은 곳에서 찍은 사진이 좋아요.</p><input ref={input} hidden type="file" accept="image/*" onChange={e => load(e.target.files?.[0])} /><div className={`drop ${image ? 'filled' : ''}`} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); load(e.dataTransfer.files[0]); }}>{image ? <img src={image} alt="업로드한 아이의 그림" /> : <><span><ImagePlus /></span><b>여기에 그림을 놓아 주세요</b><small>또는 아래 버튼으로 사진을 골라요</small></>}</div><div className="upload-buttons"><Button secondary onClick={() => input.current?.click()}><Upload size={19} /> {image ? '다른 그림 고르기' : '그림 파일 고르기'}</Button><Button secondary onClick={() => input.current?.click()}><Camera size={19} /> 사진 찍기</Button></div>{image && <strong className="quality"><Check /> 그림이 선명하고 잘 보이네요!</strong>}<Button disabled={!image} onClick={() => setStep('character')}><Sparkles size={19} /> 캐릭터로 탄생시키기</Button><small className="privacy"><LockKeyhole size={14} /> 이 데모의 그림은 브라우저 안에서만 보여요.</small></section>}

    {step === 'character' && <section className="center characters"><span className="badge">짜잔! 그림친구가 태어났어요</span><h2>어떤 모습이 가장 마음에 드나요?</h2><p>원래 그림의 색과 표정, 특별한 모양을 그대로 간직했어요.</p><div className="candidates">{['그림 그대로', '말랑 캐릭터', '쪼꼬미 인형'].map((name, i) => <button className={pick === i ? 'selected' : ''} key={name} onClick={() => setPick(i)}>{pick === i && <i className="check"><Check /></i>}<span><Friend image={image} variant={`v${i}`} /></span><b>{name}</b><small>{i === 0 ? '내 선을 가장 많이 살렸어요' : i === 1 ? '포근하고 둥근 느낌이에요' : '작은 몸에 표정이 커요'}</small></button>)}</div><div className="name"><label>친구 이름<input defaultValue="별콩이" /></label><span>용감해요</span><span>다정해요</span><span>별빛 능력</span></div><Button onClick={() => { setScene(0); setStep('adventure'); }}>별콩이와 모험 떠나기 <ChevronRight /></Button></section>}

    {step === 'adventure' && <section className="adventure"><div className="adventure-meta"><b>달빛 숲의 잃어버린 조각</b><span>{scene + 1} / 5 장면 · 약 {duration}</span></div><div className={`scene s${scene}`}><i className="moon">☾</i><i className="stars">✦　·　✧</i><div className="scene-friend"><Friend image={image} /></div><article><span>{scenes[scene][0]}</span><h2>{scenes[scene][1]}</h2><p>{scenes[scene][2]}</p></article></div><div className="choice-card"><div className="says"><Volume2 /> “{scene === 4 ? '우리가 해냈어! 오늘의 모험을 책으로 남기자.' : '좋은 생각이야! 어떤 방법으로 해 볼까?'}”</div><div className="choices">{scenes[scene][3].map((x, i) => <button key={x} onClick={() => scene < 4 ? setScene(scene + 1) : setStep('book')}><i>{String.fromCharCode(65 + i)}</i>{x}<ChevronRight /></button>)}</div><button className="speak"><Mic /> 누르고 말해서 다른 방법 알려주기</button><small className="event"><Sparkles /> 안전한 이야기 사건 {scene + 1}개가 기록되었어요</small></div></section>}

    {step === 'book' && <section className="book"><div className="book-title"><span className="badge"><BookOpen /> 모험 완성!</span><h2>우리의 첫 동화책이 만들어졌어요</h2><p>자유 대화가 아닌 안전하게 확인된 이야기만 담았어요.</p></div><div className="book-shell"><button disabled={page === 0} onClick={() => setPage(page - 1)}><ArrowLeft /></button><article><small>{pages[page][0]}</small><div className="book-art"><i>☾</i><Friend image={image} /></div><h3>{pages[page][1]}</h3><p>{pages[page][2]}</p><b>{page + 1}</b></article><button disabled={page === 5} onClick={() => setPage(page + 1)}><ChevronRight /></button></div><div className="dots">{pages.map((_, i) => <button className={i === page ? 'on' : ''} key={i} onClick={() => setPage(i)} />)}</div><div className="book-buttons"><Button secondary onClick={() => setPage(0)}><RotateCcw /> 처음부터 읽기</Button><Button onClick={() => setStep('parent')}><ShieldCheck /> 보호자에게 보여주기</Button></div></section>}
