import * as THREE from 'three';
import { createDrawingCreature } from './drawing-creature-rig';
import { DEFAULT_APPEARANCE } from './creature-types';
import type {
  CreatureAppearance,
  CreatureFrame,
  CreatureKind,
} from './creature-types';

export { DEFAULT_APPEARANCE } from './creature-types';
export type {
  CreatureAppearance,
  CreatureAction,
  CreatureFrame,
  CreatureKind,
  CreaturePattern,
  CreatureEarStyle,
} from './creature-types';

export type CreatureRig = {
  root: THREE.Group;
  update: (dt: number, time: number, input: CreatureFrame) => void;
  dispose: () => void;
  ready?: Promise<boolean>;
};

const clamp = THREE.MathUtils.clamp;
const mix = THREE.MathUtils.lerp;

// Small, object-space fibres catch light like brushed fabric. This augments
// the portable PBR material in WebGL without texture downloads or extra meshes.
function addPlushSurface(material: THREE.MeshPhysicalMaterial) {
  material.roughness = 0.89;
  material.sheen = 0.6;
  material.specularIntensity = 0.38;
  material.onBeforeCompile = (shader) => {
    shader.vertexShader =
      `varying vec3 vPlushPosition;\n${shader.vertexShader}`.replace(
        '#include <begin_vertex>',
        '#include <begin_vertex>\nvPlushPosition = position;',
      );
    shader.fragmentShader = `
      varying vec3 vPlushPosition;
      float plushHash(vec3 p) {
        p = fract(p * vec3(.1031, .11369, .13787));
        p += dot(p, p.yzx + 19.19);
        return fract((p.x + p.y) * p.z);
      }
      float plushNoise(vec3 p) {
        vec3 i = floor(p), f = fract(p);
        f = f * f * (3.0 - 2.0 * f);
        return mix(mix(mix(plushHash(i), plushHash(i + vec3(1,0,0)), f.x),
                       mix(plushHash(i + vec3(0,1,0)), plushHash(i + vec3(1,1,0)), f.x), f.y),
                   mix(mix(plushHash(i + vec3(0,0,1)), plushHash(i + vec3(1,0,1)), f.x),
                       mix(plushHash(i + vec3(0,1,1)), plushHash(i + vec3(1,1,1)), f.x), f.y), f.z);
      }
      vec3 plushNormal(vec3 surface, vec3 surfaceNormal, float height) {
        vec3 dx = dFdx(surface), dy = dFdy(surface);
        vec3 acrossX = cross(dy, surfaceNormal), acrossY = cross(surfaceNormal, dx);
        float determinant = dot(dx, acrossX);
        vec3 gradient = sign(determinant) * (dFdx(height) * acrossX + dFdy(height) * acrossY);
        return normalize(abs(determinant) * surfaceNormal - gradient);
      }
      ${shader.fragmentShader}`.replace(
      '#include <normal_fragment_maps>',
      `#include <normal_fragment_maps>
       float plushDetail = plushNoise(vPlushPosition * vec3(85.0, 112.0, 85.0));
       float plushFade = 1.0 - smoothstep(.025, .08, length(fwidth(vPlushPosition)));
       normal = plushNormal(-vViewPosition, normal, plushDetail * .0014 * plushFade);`,
    );
  };
  material.customProgramCacheKey = () => 'drawing-friend-plush-v1';
}

/** A reusable, articulated mesh companion. All features have real volume;
 * there are no billboard/image planes or remote texture dependencies. */
export function createCreature(
  appearance: CreatureAppearance = DEFAULT_APPEARANCE,
): CreatureRig {
  if (appearance.drawingImage) return createDrawingCreature(appearance);
  const root = new THREE.Group();
  root.name = 'DrawingFriend';
  root.userData = {
    assetVersion: 1,
    rigType: 'articulated',
    appearance: { ...appearance },
  };
  const geometries = new Set<THREE.BufferGeometry>();
  const materials = new Set<THREE.Material>();
  const geometry = <T extends THREE.BufferGeometry>(g: T) => {
    geometries.add(g);
    return g;
  };
  const sphere = geometry(new THREE.SphereGeometry(1, 32, 20));
  const detailSphere = geometry(new THREE.SphereGeometry(1, 20, 14));
  const makeMaterial = (
    color: THREE.ColorRepresentation,
    extra: THREE.MeshPhysicalMaterialParameters = {},
  ) => {
    const m = new THREE.MeshPhysicalMaterial({
      color,
      roughness: 0.78,
      metalness: 0,
      sheen: 0.75,
      sheenRoughness: 0.86,
      sheenColor: '#ffffff',
      ...extra,
    });
    materials.add(m);
    return m;
  };
  const fur = makeMaterial(appearance.bodyColor);
  const cream = makeMaterial(
    new THREE.Color(appearance.bodyColor).lerp(
      new THREE.Color('#fffaf1'),
      0.57,
    ),
  );
  addPlushSurface(fur);
  addPlushSurface(cream);
  const accent = makeMaterial(appearance.accentColor, { roughness: 0.67 });
  const accentLight = makeMaterial(
    new THREE.Color(appearance.accentColor).lerp(
      new THREE.Color('#eaffda'),
      0.4,
    ),
  );
  const pink = makeMaterial('#efa795');
  const innerEar = makeMaterial('#efc6b4');
  const dark = makeMaterial('#251c20', {
    roughness: 0.27,
    clearcoat: 0.32,
    clearcoatRoughness: 0.16,
    specularIntensity: 0.42,
    envMapIntensity: 0.33,
    sheen: 0,
  });
  const iris = makeMaterial('#604034', {
    roughness: 0.6,
    clearcoat: 0,
    specularIntensity: 0.08,
    envMapIntensity: 0.15,
    sheen: 0,
  });
  const pupil = makeMaterial('#251c20', {
    roughness: 0.5,
    clearcoat: 0,
    specularIntensity: 0.08,
    envMapIntensity: 0.15,
    sheen: 0,
  });
  const white = new THREE.MeshBasicMaterial({
    color: '#fffaf1',
    toneMapped: false,
  });
  materials.add(white);
  const gold = makeMaterial('#f9c571', {
    roughness: 0.3,
    metalness: 0.28,
    clearcoat: 0.4,
  });
  const seam = makeMaterial('#705947', { roughness: 0.95, sheen: 0 });
  const marking = makeMaterial(
    new THREE.Color(appearance.accentColor).lerp(
      new THREE.Color(appearance.bodyColor),
      0.12,
    ),
  );
  addPlushSurface(marking);

  function ellipsoid(
    parent: THREE.Object3D,
    name: string,
    mat: THREE.Material,
    position: [number, number, number],
    size: [number, number, number],
    detail = false,
  ) {
    const mesh = new THREE.Mesh(detail ? detailSphere : sphere, mat);
    mesh.name = name;
    mesh.position.set(...position);
    mesh.scale.set(...size);
    mesh.castShadow = true;
    mesh.receiveShadow = true;
    parent.add(mesh);
    return mesh;
  }
  function pivot(
    parent: THREE.Object3D,
    name: string,
    x: number,
    y: number,
    z: number,
  ) {
    const node = new THREE.Group();
    node.name = name;
    node.position.set(x, y, z);
    parent.add(node);
    return node;
  }
  function tube(
    parent: THREE.Object3D,
    name: string,
    points: number[][],
    radius: number,
    mat: THREE.Material,
  ) {
    const curve = new THREE.CatmullRomCurve3(
      points.map(([x, y, z]) => new THREE.Vector3(x, y, z)),
    );
    const mesh = new THREE.Mesh(
      geometry(new THREE.TubeGeometry(curve, 20, radius, 6, false)),
      mat,
    );
    mesh.name = name;
    mesh.castShadow = true;
    parent.add(mesh);
    return mesh;
  }
  // Curved appliqués follow the actual plush surface. Concentric subdivisions
  // avoid the flat polygon / floating sticker look of an ordinary ShapeGeometry.
  function surfacePatch(
    parent: THREE.Object3D,
    name: string,
    mat: THREE.Material,
    outline: (angle: number) => [number, number],
    surface: (x: number, y: number) => THREE.Vector3,
  ) {
    const segments = 48,
      rings = 10;
    const positions: number[] = [...surface(0, 0).toArray()];
    const indices: number[] = [];
    for (let ring = 1; ring <= rings; ring++) {
      for (let segment = 0; segment < segments; segment++) {
        const [x, y] = outline((segment / segments) * Math.PI * 2);
        positions.push(
          ...surface((x * ring) / rings, (y * ring) / rings).toArray(),
        );
      }
    }
    for (let segment = 0; segment < segments; segment++) {
      const next = (segment + 1) % segments;
      indices.push(0, 1 + segment, 1 + next);
      for (let ring = 1; ring < rings; ring++) {
        const a = 1 + (ring - 1) * segments + segment;
        const b = 1 + (ring - 1) * segments + next;
        const c = a + segments,
          d = b + segments;
        indices.push(a, c, b, b, c, d);
      }
    }
    const g = geometry(new THREE.BufferGeometry());
    g.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
    g.setIndex(indices);
    g.computeVertexNormals();
    // Outlines can have either winding (the heart runs clockwise). Keep the
    // outward-facing normal consistent without using expensive double-sided fur.
    if (g.attributes.normal.getZ(0) < 0) {
      for (let i = 0; i < indices.length; i += 3)
        [indices[i + 1], indices[i + 2]] = [indices[i + 2], indices[i + 1]];
      g.setIndex(indices);
      g.computeVertexNormals();
    }
    const mesh = new THREE.Mesh(g, mat);
    mesh.name = name;
    mesh.receiveShadow = true;
    parent.add(mesh);
    return mesh;
  }
  const circle =
    (rx: number, ry: number) =>
    (angle: number): [number, number] => [
      Math.cos(angle) * rx,
      Math.sin(angle) * ry,
    ];
  function tummySurface(cx: number, cy: number) {
    return (x: number, y: number) => {
      const px = x + cx,
        py = y + cy;
      const z =
        0.335 +
        0.135 *
          Math.sqrt(
            Math.max(0, 1 - (px / 0.37) ** 2 - ((py + 0.045) / 0.4) ** 2),
          );
      return new THREE.Vector3(px, py, z + 0.0025);
    };
  }
  const bodyPivot = pivot(root, 'BodyPivot', 0, 0.83, 0);
  ellipsoid(bodyPivot, 'PearBody', fur, [0, 0, 0], [0.57, 0.66, 0.43]);
  ellipsoid(
    bodyPivot,
    'SoftTummy',
    cream,
    [0, -0.045, 0.335],
    [0.37, 0.4, 0.135],
  );
  if (appearance.pattern === 'heart') {
    surfacePatch(
      bodyPivot,
      'HeartTummy',
      marking,
      (angle) => [
        ((16 * Math.sin(angle) ** 3) / 18) * 0.23,
        ((13 * Math.cos(angle) -
          5 * Math.cos(2 * angle) -
          2 * Math.cos(3 * angle) -
          Math.cos(4 * angle)) /
          18) *
          0.23,
      ],
      tummySurface(0, -0.12),
    );
  } else if (appearance.pattern === 'spots') {
    surfacePatch(
      bodyPivot,
      'TummySpotL',
      marking,
      circle(0.079, 0.092),
      tummySurface(-0.14, -0.055),
    );
    surfacePatch(
      bodyPivot,
      'TummySpotR',
      marking,
      circle(0.092, 0.085),
      tummySurface(0.12, -0.18),
    );
    surfacePatch(
      bodyPivot,
      'TummySpotTiny',
      marking,
      circle(0.049, 0.045),
      tummySurface(-0.066, -0.285),
    );
  }
  const head = pivot(bodyPivot, 'HeadPivot', 0, 0.88, 0.015);
  const headGeometry = geometry(new THREE.SphereGeometry(1, 40, 28));
  const headPositions = headGeometry.attributes.position;
  // Sculpt the lower cheeks into one continuous surface, with no attached lobes.
  for (let i = 0; i < headPositions.count; i++) {
    const x = headPositions.getX(i),
      y = headPositions.getY(i),
      z = headPositions.getZ(i);
    const lowerCheek = Math.sin(clamp(-y, 0, 1) * Math.PI);
    const cheekFront =
      0.08 *
      Math.exp(-((Math.abs(x) - 0.52) ** 2 / 0.08 + (y + 0.3) ** 2 / 0.12)) *
      Math.max(z, 0);
    headPositions.setXYZ(
      i,
      x * (1 + lowerCheek * 0.09 - Math.max(y, 0) * 0.035),
      y,
      z * (1 + lowerCheek * 0.035) + cheekFront,
    );
  }
  headGeometry.computeVertexNormals();
  const headNormals = headGeometry.attributes.normal;
  for (let i = 0; i < headNormals.count; i++) {
    const normal = new THREE.Vector3().fromBufferAttribute(headNormals, i);
    if (normal.lengthSq() < 0.5) normal.fromBufferAttribute(headPositions, i);
    normal.normalize();
    headNormals.setXYZ(i, normal.x, normal.y, normal.z);
  }
  const headMesh = new THREE.Mesh(headGeometry, fur);
  headMesh.name = 'CloudHead';
  headMesh.scale.set(0.86, 0.7, 0.64);
  headMesh.castShadow = true;
  headMesh.receiveShadow = true;
  head.add(headMesh);
  if (appearance.pattern === 'spots') {
    surfacePatch(
      head,
      'ForeheadSpot',
      marking,
      circle(0.17, 0.14),
      (dx, dy) => {
        const x = dx - 0.53,
          y = dy + 0.48;
        const z = Math.sqrt(Math.max(0, 1 - x * x - y * y));
        // Same sculpt as CloudHead, in final scaled coordinates.
        const lowerCheek = Math.sin(clamp(-y, 0, 1) * Math.PI);
        const cheekFront =
          0.08 *
          Math.exp(
            -((Math.abs(x) - 0.52) ** 2 / 0.08 + (y + 0.3) ** 2 / 0.12),
          ) *
          Math.max(z, 0);
        return new THREE.Vector3(
          x * (1 + lowerCheek * 0.09 - Math.max(y, 0) * 0.035) * 0.86,
          y * 0.7,
          (z * (1 + lowerCheek * 0.035) + cheekFront) * 0.64 + 0.003,
        );
      },
    );
  }
  ellipsoid(
    head,
    'MuzzleL',
    cream,
    [-0.113, -0.25, 0.592],
    [0.19, 0.139, 0.095],
    true,
  );
  ellipsoid(
    head,
    'MuzzleR',
    cream,
    [0.113, -0.25, 0.592],
    [0.19, 0.139, 0.095],
    true,
  );
  ellipsoid(
    head,
    'BlushL',
    pink,
    [-0.53, -0.17, 0.539],
    [0.135, 0.083, 0.023],
    true,
  ).rotation.y = -0.3;
  ellipsoid(
    head,
    'BlushR',
    pink,
    [0.53, -0.17, 0.539],
    [0.135, 0.083, 0.023],
    true,
  ).rotation.y = 0.3;

  const eyes: THREE.Group[] = [];
  for (const side of [-1, 1]) {
    const eye = pivot(
      head,
      side === -1 ? 'EyeL' : 'EyeR',
      side * 0.3,
      0.032,
      0.572,
    );
    eye.rotation.y = side * 0.13;
    ellipsoid(
      eye,
      `EyeWhite${side}`,
      cream,
      [0, 0, -0.008],
      [0.144, 0.172, 0.066],
      true,
    );
    ellipsoid(
      eye,
      `EyeGlass${side}`,
      dark,
      [0, 0, 0.024],
      [0.126, 0.154, 0.087],
      true,
    );
    ellipsoid(
      eye,
      `WarmIris${side}`,
      iris,
      [0, -0.048, 0.096],
      [0.079, 0.058, 0.019],
      true,
    );
    ellipsoid(
      eye,
      `EyePupil${side}`,
      pupil,
      [0, 0.02, 0.092],
      [0.092, 0.108, 0.031],
      true,
    );
    ellipsoid(
      eye,
      `EyeCatchlight${side}`,
      white,
      [-0.04, 0.06, 0.119],
      [0.04, 0.045, 0.014],
      true,
    );
    ellipsoid(
      eye,
      `EyeSpark${side}`,
      white,
      [0.046, -0.045, 0.12],
      [0.016, 0.017, 0.009],
      true,
    );
    // Layered eye details must not shadow each other: that produces glittery
    // shadow acne at mobile sizes instead of a single clear expression.
    eye.traverse((node) => {
      if (node instanceof THREE.Mesh) {
        node.castShadow = false;
        node.receiveShadow = false;
      }
    });
    eyes.push(eye);
  }
  ellipsoid(
    head,
    'ButtonNose',
    pink,
    [0, -0.152, 0.68],
    [0.079, 0.06, 0.052],
    true,
  );
  ellipsoid(
    head,
    'NoseHighlight',
    cream,
    [-0.018, -0.133, 0.727],
    [0.017, 0.01, 0.007],
    true,
  );
  const smile = tube(
    head,
    'Smile',
    [
      [-0.125, -0.287, 0.674],
      [-0.065, -0.324, 0.694],
      [0, -0.337, 0.696],
      [0.065, -0.324, 0.694],
      [0.125, -0.287, 0.674],
    ],
    0.012,
    seam,
  );
  const openMouth = pivot(head, 'OpenMouth', 0, -0.307, 0.693);
  ellipsoid(
    openMouth,
    'MouthInterior',
    dark,
    [0, 0, 0],
    [0.07, 0.079, 0.018],
    true,
  );
  ellipsoid(
    openMouth,
    'TinyTongue',
    pink,
    [0, -0.035, 0.02],
    [0.045, 0.022, 0.009],
    true,
  );
  openMouth.scale.setScalar(0.001);

  const floppy = appearance.earStyle === 'floppy';
  const earPivots: THREE.Group[] = [];
  function addLopEar(ear: THREE.Group, side: number) {
    const long = appearance.kind === 'bunny';
    const length = long ? 1 : 0.68;
    const width = long ? 0.178 : 0.216;
    const depth = long ? 0.112 : 0.134;
    const curve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(0, -0.06, 0),
      new THREE.Vector3(side * 0.17 * length, 0.15 * length, 0.025),
      new THREE.Vector3(side * 0.36 * length, 0.06 * length, 0.07),
      new THREE.Vector3(side * 0.4 * length, -0.24 * length, 0.13),
      new THREE.Vector3(side * 0.33 * length, -0.45 * length, 0.15),
    ]);
    function bend(x: number, y: number, z: number) {
      const u = clamp((y + 1) / 2, 0, 1);
      const center = curve.getPoint(u),
        tangent = curve.getTangent(u);
      // The transverse direction follows the whole curved ear, making one
      // continuous silhouette rather than a chain of disconnected spheres.
      const transverse = new THREE.Vector3(
        tangent.y,
        -tangent.x,
        0,
      ).normalize();
      return center
        .addScaledVector(transverse, x * width)
        .add(new THREE.Vector3(0, 0, z * depth));
    }
    const g = geometry(new THREE.SphereGeometry(1, 28, 24));
    const p = g.attributes.position;
    for (let i = 0; i < p.count; i++) {
      const v = bend(p.getX(i), p.getY(i), p.getZ(i));
      p.setXYZ(i, v.x, v.y, v.z);
    }
    g.computeVertexNormals();
    const mesh = new THREE.Mesh(g, fur);
    mesh.name = `LopEar${side}`;
    mesh.castShadow = true;
    mesh.receiveShadow = true;
    ear.add(mesh);
    surfacePatch(
      ear,
      `LopEarInner${side}`,
      innerEar,
      circle(0.56, 0.81),
      (x, y) => {
        const v = bend(x, y, Math.sqrt(Math.max(0, 1 - x * x - y * y)));
        v.z += 0.003;
        return v;
      },
    );
  }
  for (const side of [-1, 1]) {
    const ear = pivot(
      head,
      side === -1 ? 'EarL' : 'EarR',
      side * 0.57,
      0.44,
      -0.055,
    );
    ear.rotation.z = floppy ? 0 : side * -0.23;
    if (floppy && appearance.kind !== 'cat') {
      addLopEar(ear, side);
    } else if (appearance.kind === 'bunny') {
      ellipsoid(ear, `BunnyEar${side}`, fur, [0, 0.28, 0], [0.18, 0.49, 0.145]);
      ellipsoid(
        ear,
        `BunnyEarInner${side}`,
        innerEar,
        [0, 0.29, 0.115],
        [0.1, 0.355, 0.045],
        true,
      );
    } else if (appearance.kind === 'cat' && floppy) {
      // A folded cat ear is a thick, closed cushion, not a flattened extruded
      // triangle. Bend a tapered sphere gently forward so every viewing angle
      // retains a soft rim and rounded tip, with no hard crease or hollow back.
      const foldPoint = (x: number, y: number, z: number) => {
        const upper = Math.max(0, y),
          bend = upper * upper;
        return new THREE.Vector3(
          x * 0.22 * (1 - upper * 0.3),
          0.075 + y * 0.23 - bend * 0.11,
          0.025 + z * 0.17 + bend * 0.18,
        );
      };
      const g = geometry(new THREE.SphereGeometry(1, 32, 24));
      const positions = g.attributes.position;
      const originalNormals = g.attributes.normal.clone();
      for (let i = 0; i < positions.count; i++) {
        const p = foldPoint(
          positions.getX(i),
          positions.getY(i),
          positions.getZ(i),
        );
        positions.setXYZ(i, p.x, p.y, p.z);
      }
      g.computeVertexNormals();
      const normals = g.attributes.normal;
      for (let i = 0; i < normals.count; i++) {
        const normal = new THREE.Vector3().fromBufferAttribute(normals, i);
        if (normal.lengthSq() < 0.5)
          normal.fromBufferAttribute(originalNormals, i);
        normal.normalize();
        normals.setXYZ(i, normal.x, normal.y, normal.z);
      }
      const mesh = new THREE.Mesh(g, fur);
      mesh.name = `FoldedCatEar${side}`;
      mesh.castShadow = true;
      mesh.receiveShadow = true;
      ear.add(mesh);
      surfacePatch(
        ear,
        `FoldedCatEarInner${side}`,
        innerEar,
        circle(0.54, 0.59),
        (x, y) => {
          const py = y - 0.07;
          const p = foldPoint(
            x,
            py,
            Math.sqrt(Math.max(0, 1 - x * x - py * py)),
          );
          p.z += 0.003;
          return p;
        },
      );
    } else if (appearance.kind === 'cat') {
      const shape = new THREE.Shape();
      shape.moveTo(-0.2, -0.07);
      shape.quadraticCurveTo(-0.16, 0.2, -0.035, 0.37);
      shape.quadraticCurveTo(0.005, 0.42, 0.05, 0.35);
      shape.quadraticCurveTo(0.22, 0.15, 0.22, -0.07);
      shape.closePath();
      const g = geometry(
        new THREE.ExtrudeGeometry(shape, {
          depth: 0.12,
          bevelEnabled: true,
          bevelSegments: 3,
          steps: 1,
          bevelSize: 0.065,
          bevelThickness: 0.06,
          curveSegments: 10,
        }),
      );
      const mesh = new THREE.Mesh(g, fur);
      mesh.name = `CatEar${side}`;
      mesh.castShadow = true;
      mesh.position.z = -0.12;
      ear.add(mesh);
      ellipsoid(
        ear,
        `CatEarInner${side}`,
        innerEar,
        [0, 0.12, 0.071],
        [0.1, 0.18, 0.04],
        true,
      );
    } else {
      ellipsoid(
        ear,
        `RoundEar${side}`,
        fur,
        [0, 0.065, 0],
        [0.235, 0.235, 0.145],
      );
      ellipsoid(
        ear,
        `RoundEarInner${side}`,
        innerEar,
        [0, 0.07, 0.113],
        [0.128, 0.135, 0.041],
        true,
      );
    }
    earPivots.push(ear);
  }
  const tuft = pivot(head, 'LeafTuft', 0, 0.64, 0.015);
  if (appearance.kind === 'sprout') {
    tube(
      tuft,
      'LeafStem',
      [
        [0, -0.015, 0],
        [0.025, 0.125, -0.014],
        [0, 0.22, -0.018],
      ],
      0.029,
      accent,
    );
    const leafL = ellipsoid(
      tuft,
      'LeafL',
      accent,
      [-0.12, 0.19, 0],
      [0.205, 0.085, 0.125],
      true,
    );
    leafL.rotation.z = -0.43;
    const leafR = ellipsoid(
      tuft,
      'LeafR',
      accentLight,
      [0.11, 0.27, -0.018],
      [0.2, 0.077, 0.11],
      true,
    );
    leafR.rotation.z = 0.43;
  } else {
    ellipsoid(
      tuft,
      'ForeheadTuftL',
      cream,
      [-0.085, -0.042, 0.045],
      [0.14, 0.11, 0.12],
      true,
    ).rotation.z = -0.3;
    ellipsoid(
      tuft,
      'ForeheadTuftR',
      cream,
      [0.07, -0.025, 0.024],
      [0.12, 0.12, 0.12],
      true,
    ).rotation.z = 0.38;
  }

  const armL = pivot(bodyPivot, 'ArmL', -0.5, 0.21, 0.01);
  const armR = pivot(bodyPivot, 'ArmR', 0.5, 0.21, 0.01);
  armL.rotation.z = -0.11;
  armR.rotation.z = 0.11;
  for (const [arm, side] of [
    [armL, -1],
    [armR, 1],
  ] as const) {
    ellipsoid(
      arm,
      `ArmFur${side}`,
      fur,
      [side * 0.09, -0.22, 0.025],
      [0.195, 0.3, 0.19],
    );
    ellipsoid(
      arm,
      `PawPad${side}`,
      innerEar,
      [side * 0.095, -0.35, 0.175],
      [0.083, 0.065, 0.03],
      true,
    );
  }
  const legL = pivot(bodyPivot, 'LegL', -0.255, -0.42, 0.012);
  const legR = pivot(bodyPivot, 'LegR', 0.255, -0.42, 0.012);
  for (const [leg, side] of [
    [legL, -1],
    [legR, 1],
  ] as const) {
    ellipsoid(leg, `Foot${side}`, fur, [0, -0.225, 0.085], [0.205, 0.18, 0.27]);
    ellipsoid(
      leg,
      `ToeGlow${side}`,
      cream,
      [0, -0.196, 0.297],
      [0.124, 0.091, 0.043],
      true,
    );
  }
  const tail = pivot(bodyPivot, 'TailPivot', 0, -0.19, -0.37);
  if (appearance.kind === 'cat') {
    tube(
      tail,
      'CurledTail',
      [
        [0, 0, 0],
        [0.23, 0.02, -0.23],
        [0.48, 0.24, -0.3],
        [0.48, 0.49, -0.19],
        [0.33, 0.53, -0.15],
      ],
      0.095,
      fur,
    );
  } else {
    ellipsoid(tail, 'PomTail', cream, [0, 0.015, -0.12], [0.215, 0.2, 0.22]);
  }

  const accessory = pivot(bodyPivot, 'AccessoryPivot', 0, 0.345, 0.405);
  // A softly curved leaf collar gives each creature the same family identity.
  for (const side of [-1, 1]) {
    const collar = ellipsoid(
      bodyPivot,
      `LeafCollar${side}`,
      accent,
      [side * 0.215, 0.385, 0.328],
      [0.267, 0.1, 0.115],
      true,
    );
    collar.rotation.z = side * 0.39;
  }
  if (appearance.accessory === 'star') {
    const star = new THREE.Shape();
    for (let i = 0; i < 10; i++) {
      const angle = Math.PI / 2 + (i * Math.PI) / 5;
      const radius = i % 2 === 0 ? 0.157 : 0.078;
      const x = Math.cos(angle) * radius,
        y = Math.sin(angle) * radius;
      if (i === 0) star.moveTo(x, y);
      else star.lineTo(x, y);
    }
    star.closePath();
    const pendant = new THREE.Mesh(
      geometry(
        new THREE.ExtrudeGeometry(star, {
          depth: 0.045,
          bevelEnabled: true,
          bevelSegments: 3,
          bevelSize: 0.023,
          bevelThickness: 0.023,
          curveSegments: 4,
        }),
      ),
      gold,
    );
    pendant.name = 'WishStar';
    pendant.position.set(0, -0.13, 0.016);
    pendant.castShadow = true;
    accessory.add(pendant);
    ellipsoid(
      accessory,
      'GoldClasp',
      gold,
      [0, 0.045, 0.019],
      [0.042, 0.063, 0.04],
      true,
    );
  } else if (appearance.accessory === 'flower') {
    for (let i = 0; i < 5; i++) {
      const a = (i * Math.PI * 2) / 5;
      ellipsoid(
        accessory,
        `Petal${i}`,
        cream,
        [Math.sin(a) * 0.098, -0.07 + Math.cos(a) * 0.098, 0.058],
        [0.07, 0.07, 0.037],
        true,
      );
    }
    ellipsoid(
      accessory,
      'FlowerHeart',
      gold,
      [0, -0.07, 0.09],
      [0.065, 0.065, 0.042],
      true,
    );
  } else {
    const knot = ellipsoid(
      accessory,
      'ScarfKnot',
      accentLight,
      [0, -0.01, 0.05],
      [0.106, 0.085, 0.08],
      true,
    );
    knot.rotation.z = -0.2;
    ellipsoid(
      accessory,
      'ScarfEndL',
      accent,
      [-0.075, -0.18, 0.023],
      [0.09, 0.18, 0.05],
      true,
    ).rotation.z = -0.2;
    ellipsoid(
      accessory,
      'ScarfEndR',
      accentLight,
      [0.07, -0.15, 0.045],
      [0.087, 0.15, 0.05],
      true,
    ).rotation.z = 0.35;
  }

  let moveBlend = 0,
    localTime = 0,
    actionTime = 0,
    lastAction = 'idle';
  let lastActionId: number | undefined;
  let headX = 0,
    headY = 0,
    headZ = 0,
    gaitPhase = 0;
  let expressionBlend = 0;
  function update(dt: number, _time: number, input: CreatureFrame) {
    const delta = clamp(Number.isFinite(dt) ? dt : 0, 0, 0.08);
    localTime += delta;
    const action = input.action || 'idle';
    if (action !== lastAction || input.actionId !== lastActionId) {
      actionTime = 0;
      lastAction = action;
      lastActionId = input.actionId;
    }
    actionTime += delta;
    const eased = 1 - Math.exp(-delta * 10);
    moveBlend = mix(moveBlend, input.moving ? 1 : 0, eased);
    expressionBlend = mix(expressionBlend, action === 'sleep' ? 1 : 0, eased);
    const reduced = input.reducedMotion ? 0.16 : 1;
    gaitPhase += delta * (7.5 + clamp(input.speed ?? 1, 0, 3) * 3.5);
    const gait = Math.sin(gaitPhase) * moveBlend * reduced;
    const breathing = Math.sin(localTime * 2.15) * 0.014 * reduced;
    const joy = action === 'celebrate';
    const pet = action === 'pet';
    const sleeping = action === 'sleep';
    const hop =
      action === 'hop'
        ? Math.pow(
            Math.max(0, Math.sin(Math.min(actionTime / 0.8, 1) * Math.PI)),
            1.25,
          ) *
          0.47 *
          reduced
        : 0;
    const bouncing = joy
      ? Math.abs(Math.sin(actionTime * 6.5)) * 0.14 * reduced
      : 0;
    bodyPivot.position.y =
      0.83 +
      breathing +
      Math.abs(gait) * 0.055 +
      hop +
      bouncing -
      expressionBlend * 0.1;
    bodyPivot.rotation.z =
      gait * 0.048 + (pet ? Math.sin(actionTime * 5) * 0.045 * reduced : 0);
    bodyPivot.rotation.x = moveBlend * 0.07 - expressionBlend * 0.09;
    bodyPivot.scale.set(
      1 + breathing * 0.36,
      1 + breathing,
      1 + breathing * 0.6,
    );
    const curious = action === 'curious';
    const resting = action === 'idle' && !input.moving ? reduced : 0;
    const targetX =
      clamp(input.lookY ?? 0, -0.5, 0.5) * 0.21 +
      (sleeping ? 0.17 : pet ? -0.1 : 0) +
      Math.sin(localTime * 0.67) * 0.02 * resting;
    const targetY =
      clamp(input.lookX ?? 0, -1, 1) * 0.34 +
      (curious ? Math.sin(actionTime * 1.7) * 0.15 : 0) +
      Math.sin(localTime * 0.48) * 0.035 * resting;
    const targetZ = curious
      ? -0.17
      : pet
        ? Math.sin(actionTime * 2.3) * 0.09
        : Math.sin(localTime * 0.78) * 0.018 * resting;
    headX = mix(headX, targetX, eased * 0.65);
    headY = mix(headY, targetY, eased * 0.65);
    headZ = mix(headZ, targetZ, eased * 0.65);
    head.rotation.set(headX - gait * 0.025, headY, headZ - gait * 0.027);
    // Unevenly spaced single / double blinks feel less like a repeating toy loop.
    const blinkCycle = localTime % 11.6;
    const blinkAt = (start: number, duration: number) =>
      blinkCycle >= start && blinkCycle < start + duration
        ? Math.sin(((blinkCycle - start) / duration) * Math.PI)
        : 0;
    const blink = Math.max(
      blinkAt(1.9, 0.15),
      blinkAt(6.2, 0.14),
      blinkAt(6.49, 0.1),
      blinkAt(9.75, 0.16),
    );
    const happyEyes = pet ? 0.57 + Math.sin(actionTime * 3) * 0.05 : 1;
    const eyeScale = Math.max(
      0.065,
      (1 - blink * 0.93) * happyEyes * (1 - expressionBlend * 0.92),
    );
    for (const eye of eyes) eye.scale.y = eyeScale;
    const mouthIsOpen = joy || (curious && Math.sin(actionTime * 1.7) > 0.15);
    smile.visible = !mouthIsOpen;
    if (mouthIsOpen) openMouth.scale.set(joy ? 1.28 : 0.78, joy ? 0.85 : 1, 1);
    else openMouth.scale.setScalar(0.001);
    let leftX = gait * 0.62,
      rightX = -gait * 0.62;
    let leftZ = -0.11,
      rightZ = 0.11,
      rightY = 0;
    if (action === 'wave') {
      rightX = -0.12;
      rightY = Math.sin(actionTime * 6) * 0.22 * reduced;
      rightZ = 2.25 + Math.sin(actionTime * 9) * 0.24 * reduced;
      head.rotation.z -= 0.075;
    } else if (joy) {
      leftZ = -1.96 - Math.sin(actionTime * 7) * 0.18 * reduced;
      rightZ = 1.96 + Math.sin(actionTime * 7) * 0.18 * reduced;
    } else if (pet || sleeping) {
      leftX = -0.39;
      rightX = -0.39;
      leftZ = 0.13;
      rightZ = -0.13;
    } else if (hop > 0.02) {
      leftZ = -0.8;
      rightZ = 0.8;
    }
    armL.rotation.set(
      mix(armL.rotation.x, leftX, eased),
      0,
      mix(armL.rotation.z, leftZ, eased),
    );
    armR.rotation.set(
      mix(armR.rotation.x, rightX, eased),
      mix(armR.rotation.y, rightY, eased),
      mix(armR.rotation.z, rightZ, eased),
    );
    legL.rotation.x = -gait * 0.63;
    legR.rotation.x = gait * 0.63;
    legL.position.y = -0.42 + Math.max(0, gait) * 0.042;
    legR.position.y = -0.42 + Math.max(0, -gait) * 0.042;
    tail.rotation.y =
      Math.sin(localTime * (pet || joy ? 9 : 3.1)) *
      (pet || joy ? 0.4 : 0.13) *
      reduced;
    const earWiggle =
      (Math.sin(localTime * 2.6) * 0.027 + Math.abs(gait) * 0.09) * reduced;
    const earEase = 1 - Math.exp(-delta * (floppy ? 5.5 : 12));
    const earBase = floppy ? 0.035 : 0.23;
    // Soft ears lag behind steps and turns, instead of being rigidly synchronized.
    const earLag = floppy
      ? Math.sin(gaitPhase - 0.6) * moveBlend * 0.16 * reduced
      : 0;
    earPivots[0].rotation.z = mix(
      earPivots[0].rotation.z,
      earBase + earWiggle + earLag,
      earEase,
    );
    earPivots[1].rotation.z = mix(
      earPivots[1].rotation.z,
      -earBase - earWiggle + earLag * 0.78,
      earEase,
    );
    const earX = appearance.kind === 'bunny' ? -0.07 : 0;
    earPivots[0].rotation.x = mix(
      earPivots[0].rotation.x,
      earX + gait * (floppy ? 0.18 : 0.09) - headX * 0.16,
      earEase,
    );
    earPivots[1].rotation.x = mix(
      earPivots[1].rotation.x,
      -earX * 0.4 - gait * (floppy ? 0.18 : 0.09) - headX * 0.16,
      earEase,
    );
    tuft.rotation.z = Math.sin(localTime * 2.9) * 0.045 * reduced + gait * 0.05;
    accessory.rotation.z =
      Math.sin(localTime * 3.4) * 0.025 * reduced + gait * 0.09;
  }
  update(0, 0, { moving: false });
  return {
    root,
    update,
    dispose: () => {
      geometries.forEach((g) => g.dispose());
      materials.forEach((m) => m.dispose());
    },
  };
}

/** Portable animation clips use the same named pivots as the realtime rig.
 * They remain editable in a 3D authoring tool after exporting the GLB. */
export function createCreatureAnimations(): THREE.AnimationClip[] {
  const rotation = (name: string, times: number[], angles: number[][]) => {
    const values = angles.flatMap((a) =>
      new THREE.Quaternion()
        .setFromEuler(new THREE.Euler(a[0], a[1], a[2]))
        .toArray(),
    );
    return new THREE.QuaternionKeyframeTrack(
      `${name}.quaternion`,
      times,
      values,
    );
  };
  const position = (name: string, times: number[], values: number[][]) =>
    new THREE.VectorKeyframeTrack(`${name}.position`, times, values.flat());
  const idle = new THREE.AnimationClip('Breathe', 2.8, [
    position(
      'BodyPivot',
      [0, 1.4, 2.8],
      [
        [0, 0.83, 0],
        [0, 0.845, 0],
        [0, 0.83, 0],
      ],
    ),
    rotation(
      'HeadPivot',
      [0, 1.4, 2.8],
      [
        [0, -0.025, -0.018],
        [0.018, 0.025, 0.018],
        [0, -0.025, -0.018],
      ],
    ),
  ]);
  const t = [0, 0.2, 0.4, 0.6, 0.8];
  const walk = new THREE.AnimationClip('Walk', 0.8, [
    rotation('LegL', t, [
      [0, 0, 0],
      [0.52, 0, 0],
      [0, 0, 0],
      [-0.52, 0, 0],
      [0, 0, 0],
    ]),
    rotation('LegR', t, [
      [0, 0, 0],
      [-0.52, 0, 0],
      [0, 0, 0],
      [0.52, 0, 0],
      [0, 0, 0],
    ]),
    rotation('ArmL', t, [
      [0, 0, -0.11],
      [-0.4, 0, -0.11],
      [0, 0, -0.11],
      [0.4, 0, -0.11],
      [0, 0, -0.11],
    ]),
    rotation('ArmR', t, [
      [0, 0, 0.11],
      [0.4, 0, 0.11],
      [0, 0, 0.11],
      [-0.4, 0, 0.11],
      [0, 0, 0.11],
    ]),
    position('BodyPivot', t, [
      [0, 0.83, 0],
      [0, 0.88, 0],
      [0, 0.83, 0],
      [0, 0.88, 0],
      [0, 0.83, 0],
    ]),
  ]);
  const wave = new THREE.AnimationClip('Wave', 1.6, [
    rotation(
      'ArmR',
      [0, 0.3, 0.55, 0.8, 1.05, 1.3, 1.6],
      [
        [0, 0, 0.11],
        [-0.12, 0, 2.2],
        [-0.12, -0.2, 2.5],
        [-0.12, 0.2, 2.1],
        [-0.12, -0.2, 2.5],
        [-0.12, 0.2, 2.1],
        [0, 0, 0.11],
      ],
    ),
    rotation(
      'HeadPivot',
      [0, 0.35, 1.3, 1.6],
      [
        [0, 0, 0],
        [0, 0, -0.07],
        [0, 0, -0.07],
        [0, 0, 0],
      ],
    ),
  ]);
  const hop = new THREE.AnimationClip('Hop', 0.8, [
    position(
      'BodyPivot',
      [0, 0.15, 0.4, 0.65, 0.8],
      [
        [0, 0.83, 0],
        [0, 0.76, 0],
        [0, 1.3, 0],
        [0, 0.78, 0],
        [0, 0.83, 0],
      ],
    ),
    rotation(
      'ArmL',
      [0, 0.4, 0.8],
      [
        [0, 0, -0.11],
        [0, 0, -0.8],
        [0, 0, -0.11],
      ],
    ),
    rotation(
      'ArmR',
      [0, 0.4, 0.8],
      [
        [0, 0, 0.11],
        [0, 0, 0.8],
        [0, 0, 0.11],
      ],
    ),
  ]);
  const celebrate = new THREE.AnimationClip('Celebrate', 1.2, [
    rotation(
      'ArmL',
      [0, 0.3, 0.6, 0.9, 1.2],
      [
        [0, 0, -0.11],
        [0, 0, -2.2],
        [0, 0, -1.8],
        [0, 0, -2.2],
        [0, 0, -0.11],
      ],
    ),
    rotation(
      'ArmR',
      [0, 0.3, 0.6, 0.9, 1.2],
      [
        [0, 0, 0.11],
        [0, 0, 2.2],
        [0, 0, 1.8],
        [0, 0, 2.2],
        [0, 0, 0.11],
      ],
    ),
    position(
      'BodyPivot',
      [0, 0.3, 0.6, 0.9, 1.2],
      [
        [0, 0.83, 0],
        [0, 1, 0],
        [0, 0.83, 0],
        [0, 1, 0],
        [0, 0.83, 0],
      ],
    ),
  ]);
  const curious = new THREE.AnimationClip('Curious', 2, [
    rotation(
      'HeadPivot',
      [0, 0.5, 1.2, 2],
      [
        [0, 0, 0],
        [-0.03, -0.15, -0.17],
        [-0.03, 0.15, -0.17],
        [0, 0, 0],
      ],
    ),
  ]);
  const eyeTrack = (name: string, duration: number, y: number) =>
    new THREE.VectorKeyframeTrack(
      `${name}.scale`,
      [0, 0.2, duration - 0.2, duration],
      [1, 1, 1, 1, y, 1, 1, y, 1, 1, 1, 1],
    );
  const pet = new THREE.AnimationClip('Pet', 1.8, [
    eyeTrack('EyeL', 1.8, 0.5),
    eyeTrack('EyeR', 1.8, 0.5),
    rotation(
      'HeadPivot',
      [0, 0.5, 0.9, 1.4, 1.8],
      [
        [0, 0, 0],
        [-0.1, 0, -0.07],
        [-0.1, 0, 0.07],
        [-0.1, 0, -0.07],
        [0, 0, 0],
      ],
    ),
  ]);
  const sleep = new THREE.AnimationClip('Sleep', 3, [
    eyeTrack('EyeL', 3, 0.07),
    eyeTrack('EyeR', 3, 0.07),
    rotation(
      'HeadPivot',
      [0, 0.5, 2.5, 3],
      [
        [0, 0, 0],
        [0.16, 0, -0.06],
        [0.16, 0, -0.06],
        [0, 0, 0],
      ],
    ),
  ]);
  return [idle, walk, wave, hop, celebrate, curious, pet, sleep];
}

export const CREATURE_VARIANTS: Record<CreatureKind, CreatureAppearance> = {
  sprout: DEFAULT_APPEARANCE,
  bunny: {
    kind: 'bunny',
    bodyColor: '#fff0e5',
    accentColor: '#dba5ac',
    accessory: 'flower',
  },
  cat: {
    kind: 'cat',
    bodyColor: '#efd0a6',
    accentColor: '#94b9c9',
    accessory: 'scarf',
  },
  bear: {
    kind: 'bear',
    bodyColor: '#ead4c4',
    accentColor: '#adacd4',
    accessory: 'star',
  },
};
